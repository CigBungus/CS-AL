...
def CreateNewField():
  # makes the constants global so they can be used in the function
  global FIELDWIDTH
  global FIELDLENGTH
  
  run = True
  # loop to run try except
  while run:
    try:
      #asks the user to enter the x and y of field 
      FIELDWIDTH = int(input("enter width of field: "))
      FIELDLENGTH = int(input("enter length of field: "))
      # if statement to check whether x and y are within range
      if FIELDWIDTH < 11 or FIELDWIDTH > 51:
        print("the width needs to be a minimum of 10 and a maximum of 50")
      elif FIELDLENGTH < 11 or FIELDWIDTH > 51:
        print("the width needs to be a minimum of 10 and a maximum of 50")
      else:
        # if the values are within range the field will be generated
        run = False
    # if the values arent int this will pop up 
    except ValueError:
      print("you need to enter an intager")
